...
A one-pass Claude translation produces a translation. A two-pass system produces a translation that has been checked against itself. The first call translates. The second call compares the translation against the original and reports omissions.
...
- Zod validation on all AI output with retry logic on Claude failures
...
- Result badges: THOROUGH, PARTIAL, QUICK CHECK
...
- Zod schema validation with retry on Claude failures
- Rate limiting on translate, verify, share, and parse routes
...
| Verification output | Issue cards with THOROUGH / PARTIAL / QUICK CHECK badges | Result is surfaced as actionable items. No auto-patch. User decides next action. |
...
| Rate limiting | Applied to translate, verify, share, and parse routes | Abuse resistance for a free, no-login tool. Invisible to users until a wall is hit. |
| Zod validation | Applied to all Claude API responses | Schema validation with retry logic on failure. Prevents silent empty renders. |
...
| AI verification pass | Working | User-initiated. Issue cards with THOROUGH / PARTIAL / QUICK CHECK badges. No auto re-render. |
...
| Zod validation and retry | Working | All Claude responses validated. Retry on failure before error state surfaces. |
| Rate limiting | Working | Applied to translate, verify, share, and parse routes. |
...
- **Translation engine:** Twelve languages. Simple, Clear, Complete reading levels. Medical term explanation. Attribution language. Claude Sonnet (`claude-sonnet-4-20250514`). Zod validation with retry.
- **Verification:** User-initiated second Claude pass. Issue cards. THOROUGH / PARTIAL / QUICK CHECK badges. No auto re-render.
...
### Stack highlighted
Claude API (two-pass architecture, `claude-sonnet-4-20250514`), Zod (schema validation with retry), Supabase (90-day sessions, versioned migrations)
